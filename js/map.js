    ...
    try {
        ...
        mapCabins.forEach(cabin => {
            ...
            if (matched.length > 0) {
                ...
                // 檢查是否所有組別都已完成（landed 或 departed）
                allCompleted = matched.every(g => {
                ...
                // 若非全部完成，overallEnd 保持 null（顯示「進行中」）
            }

            // ★ 儲存到 cabin.fields（供顯示用）
...
// ---- 載入車廂組別狀態 (使用 getGroupStatus 個別狀態) ----
